-- normalise_epoch
generate("normalise_epoch()")
local status, message = pcall(normalise_epoch)
generate("`normalise_epoch()` -> error"..(status and "PASS" or "FAIL"))
generate("normalize_epoch error message = "..tostring(status))
generate("status="..tostring(status))
generate(message)
