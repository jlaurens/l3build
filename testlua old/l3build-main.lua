...
-- List all modules
local function listmodules (exclmodules)
  local modules = { }
  local exclmodules = exclmodules or { }
  for entry in L3.lfs.dir(".") do
    if entry ~= "." and entry ~= ".." then
      local attr = L3.lfs.attributes(entry)
    ...
    tag =
      {
        bundle_func = function(names)
            local modules = modules or listmodules(exclmodules)
            local errorlevel = L3:call(modules,"tag")
...
L3.main = L3.main or function (self)
  -- Deal with unknown targets up-front
  local target = self.options.target
  local names = self.options.name
  if not self.target_list[target] then
    self.help()
    os.exit(1)
  end
  local errorlevel = 0
  if module == "" then
    modules = modules or listmodules(exclmodules)
    if target_list[target].bundle_func then
      ...
      errorlevel = L3:call(modules,target)
    end
  else
