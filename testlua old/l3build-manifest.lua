...
manifest = manifest or function()
  ...
  ctanfiles = {}
  for _,f in ipairs(FF.filelist(ctandir.."/"..ctanpkg,"*.*")) do
    ctanfiles[f] = true
  end
  tdsfiles = {}
  for _,subdir in ipairs({"/doc/","/source/","/tex/"}) do
    for _,f in ipairs(FF.filelist(tdsdir..subdir..moduledir,"*.*")) do
      tdsfiles[f] = true
    end
  end
  ...
  if not(entry.subheading) then
    ...
    for _,glob_list in ipairs(entry.exclude) do
      for _,this_glob in ipairs(glob_list) do
        for _,this_file in ipairs(FF.filelist(maindir,this_glob)) do
    ...
    for _,glob_list in ipairs(entry.files) do
      for _,this_glob in ipairs(glob_list) do
        ...
        local these_files = FF.filelist(entry.dir,this_glob)
